...
export async function POST(req: Request) {
  try {
    const prompt = `
      ...
      who would it be?||What's a simple thing that makes you happy?'. Ensure the questions are intriguing, 
      foster curiosity, and contribute to a positive and welcoming conversational environment.
    `;

    const result = streamText({
      model: openai('gpt-4o'), // Define the model correctly
      messages: [
        { role: 'system', content: 'You are a helpful assistant for generating engaging questions.' },
